...
pub fn main(init: std.process.Init) !void {
    ...
    const total = header.HEADER_SIZE + bytes.len;
    const buf = try allocator.alloc(u8, total);
    defer allocator.free(buf);

    const pkt = try header.buildPacket(buf, src, dst, conn_id, .Data, bytes);

    std.debug.print("HEADER HEX:\n", .{});
    for (pkt[0..header.HEADER_SIZE], 0..) |b, i| {
        std.debug.print("{d:0>3} ", .{b});
        if ((i + 1) % 8 == 0) std.debug.print("\n", .{});
    }
    std.debug.print("\nPaket gebaut: {d} Bytes\n", .{pkt.len});

    const parsed = try header.parsePacket(pkt);
